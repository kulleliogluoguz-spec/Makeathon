# MASTER PROMPT: Facebook Messenger Integration

## CRITICAL RULES
1. ADDITIVE only. Do NOT rewrite existing files.
2. FIRST run: `git add -A && git commit -m "checkpoint before messenger"` as safety checkpoint.
3. Do NOT push to git.
4. Do NOT touch persona builder, voice builder, catalog manager, Instagram webhook, livechat, or existing pages.

## WHAT THIS DOES

Adds Facebook Messenger as a channel. When someone messages your Facebook Page on Messenger, the AI assistant responds with the same persona, catalog, intent scoring, and category tagging as Instagram and Live Chat. Conversations appear in the dashboard with channel="messenger".

## HOW MESSENGER WORKS (very similar to Instagram)

1. User sends message to your Facebook Page on Messenger
2. Meta sends POST to your webhook (same format as Instagram)
3. Backend generates AI reply using persona + catalog
4. Backend sends reply via Graph API
5. Conversation saved with channel="messenger"

The webhook payload format is nearly identical to Instagram — the key difference:
- Instagram: `data.object == "instagram"`
- Messenger: `data.object == "page"`
- Reply URL: `https://graph.facebook.com/v21.0/me/messages` (not graph.instagram.com)

## BACKEND — NEW FILE

### New file: `backend/app/api/messenger.py`
...
Add these lines:
```
MESSENGER_PAGE_ACCESS_TOKEN=paste-your-page-token-here
MESSENGER_VERIFY_TOKEN=bigm_verify_123
```
...
1. Go to developers.facebook.com → BIGM app
2. Left menu → **Messenger** → **Messenger API Ayarları** (Settings)
3. Find the **Webhooks** section
4. Callback URL: `https://forsakenly-kinglike-thiago.ngrok-free.dev/api/v1/messenger/webhook`
5. Verify Token: `bigm_verify_123`
6. Click Verify and Save
7. Subscribe to the **messages** field
8. In the **Access Tokens** section, generate a Page Access Token for your Facebook Page
9. Copy that token → paste it in `.env` as `MESSENGER_PAGE_ACCESS_TOKEN`

NOTE: You need a Facebook Page connected to the BIGM app. If you don't have one:
- Go to facebook.com → Create Page → business name → create
- Go back to BIGM app → Messenger settings → connect the page

## TEST PLAN

1. Checkpoint: `git add -A && git commit -m "checkpoint before messenger"`
2. Apply changes. Restart backend.
3. Test webhook:
```bash
curl "http://localhost:8000/api/v1/messenger/webhook?hub.mode=subscribe&hub.verify_token=bigm_verify_123&hub.challenge=test"
```
Should return `test`.

4. Configure webhook in Meta Dashboard (steps above).
5. From your personal Facebook account, go to Messenger, find your business Page, send a message.
6. AI should reply with persona-based response.
7. Check /conversations dashboard → new conversation with channel="messenger" and Ⓜ️ icon.
8. Check /customers → new customer with source="messenger".
...
- backend/.env (2 lines)
- backend/app/core/config.py (2 fields)

## DO NOT
- ❌ DO NOT rewrite any existing file
- ❌ DO NOT modify Instagram webhook
- ❌ DO NOT touch frontend (messenger conversations automatically appear in existing dashboard)
- ❌ DO NOT push to git
